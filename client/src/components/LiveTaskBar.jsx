    ...
    <>
      {showBanner && isNow && (
        <div
          ...
          style={{
            ...
            background: accent, padding: '12px 16px 10px',
        ...
        >
          <img src={currentSrc} width={32} height={32} alt="" style={{ flexShrink: 0, objectFit: 'contain', aspectRatio: '1' }} />
          <div style={{ flex: 1 }}>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: isNow ? `0 4px 18px ${accent}55` : '0 2px 10px rgba(70,52,28,.08)',
          animation: pulse ? 'prudencefloat 0.4s ease-in-out 3' : 'none',
          transition: 'all 0.3s',
        }}
      >
        <img src={currentSrc} width={28} height={28} alt="" style={{ flexShrink: 0, objectFit: 'contain', aspectRatio: '1', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,.15))' }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase', color: isNow ? 'rgba(255,255,255,.8)' : 'var(--ink-3)', marginBottom: 1 }}>
            {isNow ? '⏰ Starting now' : isPast ? 'Next task' : minUntil <= 60 ? `In ${minUntil}m` : upcoming.time}
          </div>
          <div style={{ fontSize: 14, fontWeight: 700, color: isNow ? '#fff' : 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {upcoming.title}
          </div>
        </div>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: isNow ? 'rgba(255,255,255,.7)' : accent, flexShrink: 0 }} />
      </div>
    </>
  );
}
